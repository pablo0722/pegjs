...
function generateJS(ast, options) {
  ...
  function generateRuleFunction(rule) {
      ...
      while (ip < end) {
        switch (bc[ip]) {
          ...
          case op.LOAD_SAVED_POS:     // LOAD_SAVED_POS p
            ...
            parts.push('verbose (`${tabs.join("")}[OK] Parse "' + rule.name + '"`);');
            ip += 2;
            break;
    ...
    parts.push('  verbose (`${tabs.join("")}[START] Parse "' + rule.name + '"`);');
    parts.push('  tabs.push("  ")');
    parts.push('');
    ...
    parts.push('');
    parts.push('  tabs.pop()');
    parts.push('  verbose (`${tabs.join("")}[END] Parse "' + rule.name + '" s0: ${JSON.stringify (s0)}`);');
